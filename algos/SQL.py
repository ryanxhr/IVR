...
def sparse(x, expectile=1.0):
    weight = torch.where(x > 0, expectile, (1.0 - expectile))
...
class SQL(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        if self.total_it % 5000 == 0:
            ...
            print(f'min, mean, max q-v value is {(q-v).min()}, {(q-v).mean()}, {(q-v).max()}')
            print(f'mean sp_weight is {sp_weight.mean()}')
